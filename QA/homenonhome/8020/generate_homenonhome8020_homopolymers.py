...
print('Adding to sys.path:', root_path)
...
def generate_blend_curves():
    """Generate time-series curves for all 80:20 home-nonhome blends"""
    ...
    print(f"Total possible 80:20 blends: {len(home_compostable_sus) * len(medium_disintegration_sus)}")
    ...
    for home_idx, (_, home_row) in enumerate(home_compostable_sus.iterrows()):
        ...
        for nonhome_idx, (_, nonhome_row) in enumerate(medium_disintegration_sus.iterrows()):
            ...
            # Generate synergistic curve for the non-home component (80% home, 20% non-home)
            home_fraction = 0.8  # 80% home-compostable
            ...
            plt.plot(days, curve, color=color, linewidth=2,
                    label=f"80%{home_polymer}-20%{nonhome_polymer} (90d: {curve[89]:.1f}%, max: {curve[-1]:.1f}%)")
            
            print(f"Generated blend {blend_count+1}: 80%{home_polymer}-20%{nonhome_polymer}: max={curve[-1]:.1f}%")
            blend_count += 1
    
    plt.xlabel('Time (days)')
    plt.ylabel('Disintegration (%)')
    plt.title('80:20 Home-Nonhome Blend Curves at 28°C (Synergistic Boost Effect)')
    plt.ylim(0, 100)
    ...
    # Save plot
    plt.savefig('home_nonhome8020_blends.png', dpi=300, bbox_inches='tight')
    print(f"Saved plot to: home_nonhome8020_blends.png")
